...
# user_device data import
# User Device document contains username (String), device_id (String), and access (String) fields
# Reads user_device.csv one line at a time, splits them into the data fields and inserts
# with open(RELATIVE_CONFIG_PATH + USERDEVICE_COLLECTION + '.csv', 'r') as userdevice_fh:
#     for userdevice_row in userdevice_fh:
#         userdevice_row = userdevice_row.rstrip()
#         if userdevice_row:
#             (username, device_id, access) = userdevice_row.split(',')
#         userdevice_data = {'username': username, 'device_id': device_id, 'access': access}
#
#         # This creates and return a pointer to the users collection
#         userdevice_collection = weather_dbh[USERDEVICE_COLLECTION]
#
#         # This inserts the data item as a document in the user collection
#         userdevice_collection.insert_one(userdevice_data)
